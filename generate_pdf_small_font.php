...
class PDF extends FPDF {
  function Header() {
    if ($this->page  == 1) {
      ...
      $this->Image('pdf_header2.jpg',0, 0, 200, 30);
      $this->Ln(40);
    }
  }
  function Footer() {
  ...
  foreach($pdf_data as $key => $value){
    ...
    $pdf->Cell(50, $cell_height, utf8_decode($item_date_out), 0, 0,'L', 0);
    $pdf->SetTextColor(1, 51, 106);
    $pdf->SetFontSize(10);
    $pdf->Cell(50, $cell_height, utf8_decode($value[0]), 0, 1,'L', 0); 
    $pdf->SetX(65); 
    $pdf->SetFontSize(8);
    if (strlen($value[1]) > 0){
      $pdf->MultiCell(120, 3, utf8_decode($value[1]), 0, 'L');
    }
    foreach($value[2] as $list_item){
      $pdf->SetX(65);
      $pdf->MultiCell(120, $cell_height, "- ".utf8_decode($list_item), 0, 'L');
    }
    if (count($value[3]) > 0) {
      $pdf->Ln(2);
      ...
      foreach($value[3] as $key => $value){
        $pdf->SetX(64);
        $pdf->Image('arrow.gif', ($pdf->x)+2, ($pdf->y)+1.2, 1.5, 1.5);
        $pdf->Cell(50, $cell_height, utf8_decode($value), 0, 1,'L', 0, $key);
      }
    }
    $pdf->Ln(3);
